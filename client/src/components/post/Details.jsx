...
export default function PostDetails() {
    ...
    useEffect(() => {
        ...
        async function fetchPostData() {
            try {
                const response = await requester.get(`http://localhost:3030/data/posts/${id}`, null, { signal });
                ...
                const allComments = await requester.get(`http://localhost:3030/data/comments?${searchParams.toString()}`, null, { signal });
    ...
    async function handleDelete() {
        ...
        try {
            await requester.delete(`http://localhost:3030/data/posts/${id}`, null, { headers: { "X-Authorization": token } });
            return navigate("/");
        } catch (error) {
    ...
    async function getComment(commentContent) {
        ...
        try {
            ...
            await requester.post(`http://localhost:3030/data/comments`, { ...formData, _ownerId: owner._id, avatar: owner.avatar, postId: id }, { headers: { "X-Authorization": token } });
        } catch (error) {
