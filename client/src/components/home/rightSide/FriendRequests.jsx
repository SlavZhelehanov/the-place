        ...
        if (token) {
            (async () => {
                try {
                    ...
                    let users = await requester.get("http://localhost:3030/jsonstore/users", null, { signal });
                    users = Object.values(users).filter(user => user._id !== data._id);

                    let oldFollowers = await requester.get("http://localhost:3030/jsonstore/followers", null, { signal });
